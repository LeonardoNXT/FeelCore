...
export default function AuthScreen({ endpoint }: { endpoint: string }) {
  ...
  const [isLogged, setIsLogged] = useState<Redirect | null>(null);
  ...
  useEffect(() => {
    const validadeToken = async () => {
      try {
        ...
        const role = data[1].role as Role;
        const firstName = data[0].name.split(" ")[0];
        redirect(role, firstName);
      } catch (err) {
        console.log(err);
      }
    };
  ...
  const redirect = (role: Role, firstName: string) => {
    const query = {
      name: firstName,
      route: "",
    };
  ...
  return (
    <AnimatePresence>
      {!isLogged ? (
        ...
        <RefreshComponent
          display="flex"
          route={isLogged.route}
          title={`Você será redirecionado, ${isLogged.name}`}
        />
      )}
    </AnimatePresence>
  );

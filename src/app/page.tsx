...
export default function Home() {
  ...
  return (
    ...
    >
      {!acess && !linkLogin && (
        <div className="px-4 md:px-0 fixed left-1/2 top-5 w-full translate-x-[-50%] flex justify-center z-20">
          <motion.div
            initial={{ opacity: 0, scale: 0.5 }}
            animate={{ opacity: 1, scale: 1 }}
            exit={{ opacity: 0, scale: 0.5 }}
            transition={{ duration: 0.9, type: "spring" }}
            className="md:max-w-[500px] w-full  px-2 py-2 border-1 items-center border-[#e1e1e12f] bg-[#eeeeee6c] backdrop-blur-2xl rounded-full  flex justify-between"
          >
            {receiveContent ? (
              <motion.div
